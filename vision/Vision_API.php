...
{$o=[];
foreach($rels as $r)
{if(isset($r['id'],$r['niveau']) && Vision_UniCell::isValidId((string)$r['id']))
    $o[]=['id'=>(string)$r['id'],'niveau'=>(int)$r['niveau']];}
return $o;}
...
if($ok) Vision::log('create '.$id);
else Vision::log('create failed '.$id);}, $shard);
return $ok ? $id : '';}
...
{if(!Vision_UniCell::isValidId($id)) return false;
$ok=false;
$shard=substr($id,0,8);
VisionLock::acquire(function() use($id,$data,$relations,&$ok)
{$cell=Vision_UniCell::read($id);
if(!$cell){ Vision::log("Update Error: cell $id not found"); return; }
$cell['data']=$data;
if($relations !== null)$cell['relations']=self::norm($relations);
$cell['updatedAt']=time();
$ok=Vision_UniCell::write($id,$cell);
Vision::log('update '.$id);}, $shard);
return $ok;}
...
{$n=0;
foreach(Vision_UniCell::listCells() as $_){
    $n++;
    if ($n % 200 === 0 && Fiber::getCurrent()) Fiber::suspend();
}
return $n;}
...
{$n++;
if ($n % 100 === 0 && Fiber::getCurrent()) Fiber::suspend();
$path=Vision_UniCell::path($id);
@clearstatcache(true, $path);
if(!file_exists($path)) continue;
$size=filesize($path);
if($size!==false)$total+=$size;}
return ['count'=>$n,'size'=>$total,'shards'=>count(Vision_UniCell::shards())];}
...
{if(!Vision_UniCell::isValidId($id) || !Vision_UniCell::isValidId($target)) return false;
$ok=false;
$shard=substr($id,0,8);
VisionLock::acquire(function() use($id,$target,$niveau,&$ok)
{$c=Vision_UniCell::read($id);
if(!$c){ Vision::log("Link Error: source $id not found"); return; }
$rels=$c['relations']??[];
foreach($rels as $r)
{if(($r['id']??null)===$target){$ok=true;return;}}
$rels[]=['id'=>$target,'niveau'=>$niveau];
$c['relations']=self::norm($rels);
$c['updatedAt']=time();
$ok=Vision_UniCell::write($id,$c);
Vision::log('link '.$id.' '.$target);}, $shard);
return $ok;}
...
{$c=Vision_UniCell::read($id);
if(!$c){ Vision::log("Unlink Error: source $id not found"); return; }
$rels=$c['relations']??[];
$out=[];
foreach($rels as $r)
...
$c['updatedAt']=time();
$ok=Vision_UniCell::write($id,$c);
Vision::log('unlink '.$id.' '.$target);}, $shard);
return $ok;}
...
if($d>16) return;
$n = count($seen);
if($n > 0 && $n % 20 === 0 && Fiber::getCurrent()) Fiber::suspend();
$c=Vision_UniCell::read($id);
if(!$c) return;
$seen[$id]=true;
$out[$id]=$c;
...
foreach(Vision_UniCell::listCells() as $id)
{$n++;
if ($n % 50 === 0 && Fiber::getCurrent()) Fiber::suspend();
$c=Vision_UniCell::read($id);
if($c&&$f($c)) yield $c;}}
